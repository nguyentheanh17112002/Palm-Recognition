...
def find_bl_br(thumb_cmc, tl, tr, img_width, img_height):
    ...
    if (0 <= tl[0] - perpendicular_vector[0] <= img_width) and (
        0 <= tl[1] - perpendicular_vector[1] <= img_height
    ):
    ...
    if (0 <= tr[0] - perpendicular_vector[0] <= img_width) and (
        0 <= tr[1] - perpendicular_vector[1] <= img_height
    ):
...
def preprocess_image(path):
    base_options = python.BaseOptions(
        model_asset_path="/home/anhnt596/LargeROIPalm/hand_landmarker.task"
    )
    ...
    image = mp.Image(
        ...
        data=cv2.cvtColor(original_image, cv2.COLOR_BGR2RGB),
    )
    ...
    thumb_cmc = (
        int(hand_landmarks_list[1].x * img_width),
        int(hand_landmarks_list[1].y * img_height),
    )
    top_left = (
        int(hand_landmarks_list[5].x * img_width),
        int(hand_landmarks_list[5].y * img_height),
    )
    top_right = (
        int(hand_landmarks_list[17].x * img_width),
        int(hand_landmarks_list[17].y * img_height),
    )

    bottom_left, bottom_right = find_bl_br(
        thumb_cmc, top_left, top_right, img_width, img_height
    )
...
def preprocess_name(image_name):
    match = re.match(r"(\d+)_(\d+)\.jpg", image_name.lower())
    try:
        personID = int(match.group(1)) + 2392
        image_order = int(match.group(2))
    except:
        print(match)
        raise SystemExit()

    return personID, image_order
...
if __name__ == "__main__":
    Original_path = "/home/anhnt596/LargeROIPalm/iitd/Right Hand"
